...
import { Activity, Clock, CheckCircle, Users, Search, Filter, FileSpreadsheet, FileText, Loader2, TrendingUp } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [demands, setDemands] = useState<DashboardDemand[]>([]);
  const [kpis, setKpis] = useState<KPIs>({ total: 0, emAndamento: 0, resolvidas: 0, cidadaos: 0, taxaResolucao: 0 });

  useEffect(() => {
    const fetchDashboardData = async () => {
      setLoading(true);
      try {
        // Fetch recent demands (limited to 20 for the table)
        const { data: demandasData, error: demandasError } = await supabase
          .from('demandas')
          .select('id, protocolo, nome_cidadao, tipo, bairro, status, data_criacao')
          .eq('arquivado', false)
        ...
        // KPIs from aggregate queries
        const [totalRes, andamentoRes, resolvidasRes, cidadaosRes] = await Promise.all([
          supabase.from('demandas').select('id', { count: 'exact', head: true }).eq('arquivado', false),
          supabase.from('demandas').select('id', { count: 'exact', head: true }).in('status', ['Em Análise', 'Encaminhada', 'Registrada']).eq('arquivado', false),
          supabase.from('demandas').select('id', { count: 'exact', head: true }).eq('status', 'Resolvida').eq('arquivado', false),
          supabase.from('cidadaos').select('id', { count: 'exact', head: true }),
        ]);

        const total = totalRes.count || 0;
        const emAndamento = andamentoRes.count || 0;
        const resolvidas = resolvidasRes.count || 0;
        const cidadaos = cidadaosRes.count || 0;
        const taxaResolucao = total > 0 ? Math.round((resolvidas / total) * 100) : 0;

        setKpis({ total, emAndamento, resolvidas, cidadaos, taxaResolucao });

        if (demandasData) {
          setDemands(demandasData.map((d: any) => ({
            id: d.id,
            protocol: d.protocolo,
            citizen: d.nome_cidadao || '—',
            type: d.tipo,
            neighborhood: d.bairro || '—',
            status: d.status,
            date: new Date(d.data_criacao).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }),
          })));
        }
      } catch (err: any) {
  ...
  const exportToCSV = () => {
    const headers = ['Protocolo', 'Cidadão', 'Categoria', 'Bairro', 'Status', 'Data'];
  ...
  const exportToPDF = () => {
    ...
    const headers = [['Protocolo', 'Cidadão', 'Categoria', 'Bairro', 'Status', 'Data']];
    const data = demands.map(d => [d.protocol, d.citizen, d.type, d.neighborhood, d.status, d.date]);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <span className="text-gray-500 font-medium text-sm">Carregando dados do dashboard...</span>
        </div>
      ) : (
        <>
          {/* KPI Cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-start gap-4">
              ...
              <div>
                <p className="text-gray-500 font-medium text-sm">Total de Demandas</p>
                <h3 className="text-3xl font-bold text-gray-900 mt-1">{kpis.total.toLocaleString('pt-BR')}</h3>
                <p className="text-xs text-violet-600 font-medium mt-1">No banco de dados</p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-start gap-4">
              ...
              <div>
                <p className="text-gray-500 font-medium text-sm">Em Andamento</p>
                <h3 className="text-3xl font-bold text-gray-900 mt-1">{kpis.emAndamento.toLocaleString('pt-BR')}</h3>
                <p className="text-xs text-gray-500 font-medium mt-1">Sendo tratadas</p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-start gap-4">
              ...
              <div>
                <p className="text-gray-500 font-medium text-sm">Resolvidas</p>
                <h3 className="text-3xl font-bold text-gray-900 mt-1">{kpis.resolvidas.toLocaleString('pt-BR')}</h3>
                <p className="text-xs text-emerald-600 font-medium mt-1 flex items-center gap-1">
                  <TrendingUp className="w-3.5 h-3.5" /> {kpis.taxaResolucao}% de resolução
                </p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-start gap-4">
              ...
              <div>
                <p className="text-gray-500 font-medium text-sm">Cidadãos</p>
                <h3 className="text-3xl font-bold text-gray-900 mt-1">{kpis.cidadaos.toLocaleString('pt-BR')}</h3>
                <p className="text-xs text-blue-600 font-medium mt-1">No CRM</p>
              </div>
            </div>
          </div>
                  ...
                  {filteredDemands.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-6 py-12 text-center text-gray-400 font-medium">
                        {demands.length === 0 ? 'Nenhuma demanda registrada ainda.' : 'Nenhuma demanda corresponde ao filtro.'}
                      </td>
                    </tr>
                  ) : (
